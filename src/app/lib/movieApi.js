...
export const getDailyBackground = async (content) => {
    try {
        const response = await fetch(`https://api.themoviedb.org/3/trending/${content}/day?`, options)
        ...
        if (data.results?.length > 0) {
            ...
            if (randomMovie.backdrop_path) {
                // Use a maior qualidade disponível
                return `https://image.tmdb.org/t/p/w1920_and_h1080_multi_faces${randomMovie.backdrop_path}`
            }
        }
        return null
    } catch (error) {
        console.error('Erro ao buscar o backdrop:', error)
        return null
    }
}
